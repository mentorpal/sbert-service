locals {
  ...
  prod_secret_jwt_key      = local.secret_vars.locals.prod_secret_jwt_key
}
...
inputs = {
  ...
  site_domain_name           = "sbert.mentorpal.org"
  ...
  eb_env_env_vars = {
    ...
    LOG_LEVEL_SBERT_SERVICE         = "INFO",
    API_SECRET_KEY                  = local.api_secret_key
    PROD_SECRET_JWT_KEY = local.prod_secret_jwt_key
  }
